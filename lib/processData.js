...
function ProcessData (options) {
    ...
    console.log('\n------------------------------'.green);
    console.log('   Calibrating controller...'.red);
    console.log('------------------------------'.green);
...
ProcessData.prototype.process = function (currentFrame) {
    ...
    if (!this.isReady) {
        console.log('   Calibration complete!'.green);
        console.log('---------------------------------\n'.green);
...
ProcessData.prototype.ignorePin = function (pins) {
    pins = _.isArray(pins) ? pins : [pins];
    _.each(pins, function (pin) {
        this.ignoreValues.push({pin: pin});
    }, this);
};
    ...
    frame.forEach(function (value, index) {
        ...
        if (value !== frame2[index]) {
            diff = {
                pin: index,
                value: frame[index],
                delta: frame[index] - frame2[index]
            };
        }
    }.bind(this));
